    ...
    for line in file:
        ...
        try:
            ...
            if soln == None:
                ...
                numofpuzz += 1
                avgmove += soln.num_moves
                avgstate += searcher.num_tested
        except KeyboardInterrupt:
            print(line + ': search terminated, no solution', end='\n')
        
    print()    
    print('solved ' + str(numofpuzz) + ' puzzles')
    if numofpuzz > 0:
        avgmove /= numofpuzz
        avgstate /= numofpuzz
        print('averages: ' + str(avgmove) + ' moves, ' + str(avgstate) + ' states tested')
